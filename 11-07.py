# Версия получения курсов криптовалют с табличным выводом
...
# --- Функции для получения данных курсов из API ---
...
def show_result_window(api_name, title, data, additional_currency_code, usd_to_additional_rate):
    ...
    if api_name in result_windows and result_windows[api_name].winfo_exists():
        ...
        update_result_window(result_windows[api_name], title, data, additional_currency_code)
    else:
        ...
        tree.column('price', width=40, anchor=tk.E)
        tree.column('currency', width=60, anchor=tk.CENTER)
        tree.column('price_additional', width=50, anchor=tk.E)
        tree.column('currency_additional', width=80, anchor=tk.CENTER)
...
def update_result_window(window, title, data, additional_currency_code=None):
    ...
    # Обновляем заголовок столбца дополнительной валюты, если передан код валюты
    if additional_currency_code:
        additional_currency_name = ADDITIONAL_CURRENCY_NAMES.get(additional_currency_code, additional_currency_code)
        window.tree.heading('currency_additional', text=additional_currency_name)
...
def update_gui_after_fetch(window, data, api_name, additional_currency_code, is_manual):
    ...
    if data is not None:
        ...
        update_result_window(window, window_title, data, additional_currency_code)
    else:
...
additional_currency_var = tk.StringVar(value="RUB - Российский рубль")
