...
def get_binance_data(base_currency):
    ...
    try:
        ...
        result = []
        # Исправленный подход: перебираем все возможные пары
    ...
    try:
        ...
        for crypto_code, prices in data.items():
            if crypto_code in TARGET_CRYPTOS:
                ...
                if base_currency in prices and prices[base_currency]:
                    price = prices[base_currency]
                    currency_name = FIAT_NAMES.get(base_currency, base_currency)
                    result.append({
                        'name': full_name,
                        'ticker': crypto_code,
...
def on_get_data():
    ...
    selected_base_currency_code = base_currency_var.get().split(' ')[0] # Извлекаем код валюты из строки "USD - Американский доллар"
...
base_currency_var = tk.StringVar(value="USD - Американский доллар") # Значение по умолчанию
