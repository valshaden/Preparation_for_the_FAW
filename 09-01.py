...
def get_binance_data():
    """Получаем данные по заданным криптовалютам с Binance"""
    try:
        ...
        # Определяем интересующие пары
        target_pairs = []
        for crypto in TARGET_CRYPTOS:
            target_pairs.extend([f"{crypto}USDT", f"{crypto}USDC", f"{crypto}BUSD", f"{crypto}BTC"])
        ...
        for pair in target_pairs:
            if pair in price_dict:
                ...
                # Разделяем символ на криптовалюту и валюту
                ...
                # Проверяем, что это нужная криптовалюта
...
def get_coingecko_data():
    """Получаем данные по заданным криптовалютам с CoinGecko"""
    try:
        ...
        result = []
        for coin in data:
            crypto_code = [k for k, v in coingecko_ids.items() if v == coin['id']][0]
            full_name = coin['name']
...
def get_cryptocompare_data():
    """Получаем данные по заданным криптовалютам с CryptoCompare"""
    try:
