...
def get_binance_data():
    """Получаем данные по заданным криптовалютам с Binance относительно USD"""
    try:
...
def get_coingecko_data():
    """Получаем данные по заданным криптовалютам с CoinGecko относительно USD"""
    try:
...
def get_cryptocompare_data():
    """Получаем данные по заданным криптовалютам с CryptoCompare относительно USD"""
    try:
        url = 'https://min-api.cryptocompare.com/data/pricemulti'
        params = {
            'fsyms': ','.join(TARGET_CRYPTOS), # Запрашиваем в заданном порядке
            'tsyms': BASE_CURRENCY_CODE # CryptoCompare использует код валюты напрямую
        }
        ...
        for crypto_code in TARGET_CRYPTOS:
            if crypto_code in data: # <-- Исправлено: добавлено тело условия
                ...
                if BASE_CURRENCY_CODE in data[crypto_code] and data[crypto_code][BASE_CURRENCY_CODE]:
                    price = data[crypto_code][BASE_CURRENCY_CODE]
                    # Всегда используем короткое имя базовой валюты для таблицы
                    currency_name = BASE_CURRENCY_NAME
                    result.append({
                        'name': full_name,
...
def on_get_data():
    ...
    if selected_api == "Binance":
        ...
        show_result_window("Binance", f"Binance Public API ({BASE_CURRENCY_CODE})", data)
    elif selected_api == "CoinGecko":
        data = get_coingecko_data()
        show_result_window("CoinGecko", f"CoinGecko API ({BASE_CURRENCY_CODE})", data)
    elif selected_api == "CryptoCompare":
        data = get_cryptocompare_data()
        show_result_window("CryptoCompare", f"CryptoCompare API ({BASE_CURRENCY_CODE})", data)
    else:
...
root.geometry("350x180") # Уменьшен размер окна
...
base_currency_label.pack(anchor=tk.W, pady=(0, 10))
